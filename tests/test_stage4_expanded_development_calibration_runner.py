...
class Stage4ExpandedDevelopmentCalibrationRunnerTest(unittest.TestCase):
    ...
    def _applicability(self, item_id: str, finding: str) -> tuple[str, float | None, str | None]:
    ...
    def test_exact_49_record_contract_accepts_only_synthetic_contract_input(self) -> None:
        batch = self._batch()
        validated = self._validate(batch)
        ...
        self.assertEqual(
            EXPECTED_NOT_APPLICABLE_RECORD_COUNT,
            sum(row["measurementStatus"] == "not_applicable" for row in validated["records"]),
        )

    def test_wikimedia_all_seven_metrics_are_measured(self) -> None:
        validated = self._validate(self._batch())
        rows = [row for row in validated["records"] if row["datasetItemId"] == WIKIMEDIA_ID]
        self.assertEqual(7, len(rows))
        self.assertTrue(all(row["measurementStatus"] == "measured" for row in rows))
        self.assertEqual(set(METRIC_SPECS), {row["findingType"] for row in rows})
    ...
    def test_materialized_observations_cover_beethoven_and_wikimedia_only(self) -> None:
        ...
        self.assertEqual(31, len(observations))
        self.assertEqual({BEETHOVEN_ID, WIKIMEDIA_ID}, {item.dataset_item_id for item in observations})
    ...
    def test_public_receipt_is_aggregate_only_and_cross_family_measured(self) -> None:
        ...
        self.assertEqual(31, receipt["measuredRecordCount"])
        self.assertEqual(18, receipt["notApplicableRecordCount"])
        ...
        self.assertEqual({"total": 7, "measured": 1, "notApplicable": 6}, receipt["findingCounts"]["compression"])
        for finding in ("skew", "blur", "glare", "shadow", "uneven_lighting", "noise"):
            self.assertEqual({"total": 7, "measured": 5, "notApplicable": 2}, receipt["findingCounts"][finding])
    ...
    def test_wikimedia_cannot_be_marked_not_applicable(self) -> None:
        batch = self._batch()
        row = next(row for row in batch["records"] if row["datasetItemId"] == WIKIMEDIA_ID)
