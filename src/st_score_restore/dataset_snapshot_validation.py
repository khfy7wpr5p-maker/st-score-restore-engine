...
def _validate_dataset_snapshot_integrity(
    ...
    *,
    ...
    if _match(
        snapshot["catalogSha256"], SHA, "snapshot.catalogSha256"
    ...
    for index, raw in enumerate(assignments):
        ...
        item_id = _match(
            assignment["datasetItemId"], ID, f"{where}.datasetItemId"
        )
        family = _match(
            assignment["sourceFamilyId"], ID, f"{where}.sourceFamilyId"
        )
        split = _enum(
            assignment["split"], ASSIGNED_SPLITS, f"{where}.split"
        )
        ...
        if _match(
            assignment["itemSha256"], SHA, f"{where}.itemSha256"
    ...
    if _bool(
        snapshot["heldOutFrozen"], "snapshot.heldOutFrozen"
    ) != held_out_present:
        raise DatasetManifestError(
            "snapshot.heldOutFrozen does not match assignments"
        )
    if _bool(
        snapshot["trainingUseActivated"], "snapshot.trainingUseActivated"
    ):
        raise DatasetManifestError(
    ...
    if _int(
        coverage["realItemCount"], "snapshot.coverage.realItemCount"
    ) != real_count:
        raise DatasetManifestError(
            "snapshot.coverage.realItemCount mismatch"
        )
    if _int(
        coverage["syntheticItemCount"], "snapshot.coverage.syntheticItemCount"
    ) != synthetic_count:
        raise DatasetManifestError(
    ...
    _match(
        review["reviewedBy"],
        DATASET_ACTOR_ID,
        "snapshot.review.reviewedBy",
    )
    reviewed_on = _date(
        review["reviewedOn"], "snapshot.review.reviewedOn"
    )
    _match(
    ...
    for index, assignment in enumerate(snapshot["assignments"]):
        ...
        for purpose in relevant:
            ...
            if (
                ...
                and item["retention"]["storageClass"]
