...
def finalize_item(value: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if ctx["source"] == "synthetic":
        ...
        _fields(
            synthetic,
            {
                ...
                "generatorCommit",
                "generatedOn",
                "derivationAuthorizationReference",
                "seed",
                "parameters",
            },
        ...
        _match(
            synthetic["generator"],
            CODE,
            f"{where}.syntheticGeneration.generator",
        )
        _match(
        ...
        generated_on = _date(
            synthetic["generatedOn"],
            f"{where}.syntheticGeneration.generatedOn",
        )
        derivation_reference = _match(
            synthetic["derivationAuthorizationReference"],
            EVIDENCE_ID,
            f"{where}.syntheticGeneration.derivationAuthorizationReference",
        )
        _int(synthetic["seed"], f"{where}.syntheticGeneration.seed")
        _parameter_tree(
            synthetic["parameters"],
            f"{where}.syntheticGeneration.parameters",
        )
        if ctx["parent"] is None:
            raise DatasetManifestError(
                f"{where} synthetic item requires a non-synthetic parent"
            )
    else:
    ...
    status = _enum(
        review["status"], DATASET_REVIEW_STATES, f"{where}.review.status"
    )
    reviewer = _match(
        review["reviewedBy"],
        DATASET_ACTOR_ID,
    ...
    reviewed_on = _date(
        review["reviewedOn"], f"{where}.review.reviewedOn", null=True
    )
    evidence = _match(
        review["evidenceReference"],
        EVIDENCE_ID,
        f"{where}.review.evidenceReference",
        null=True,
    )
    _code_array(review["noteCodes"], f"{where}.review.noteCodes")
    completed = status in {"approved", "rejected", "revoked"}
    if completed != (
        reviewer is not None and reviewed_on is not None and evidence is not None
    ):
        raise DatasetManifestError(
            f"{where}.review actor/date/evidence do not match status"
        )
    if not completed and any(v is not None for v in (reviewer, reviewed_on, evidence)):
        raise DatasetManifestError(
            f"{where}.review incomplete status cannot claim evidence"
        )
    granted = {
        purpose
        for purpose, permission in ctx["permissions"].items()
        if permission["status"] == "granted"
    }
    if granted and status != "approved":
        raise DatasetManifestError(
            f"{where} active permission requires approved dataset review"
        )
    if ctx["artifact"] == "external_available" and status != "approved":
        raise DatasetManifestError(
            f"{where} external artifact requires approved dataset review"
        )
    ...
    names = {
        ...
        "originalBytesInGit",
        "stage1TrainingExecutionAuthorized",
    }
