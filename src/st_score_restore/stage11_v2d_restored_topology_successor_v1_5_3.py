"""Stage 11 V2d restored-output topology successor v1.5.3.

Development-only minor successor to v1.5.2. Frozen v1.4, v1.5, v1.5.1 and
failed-development candidate v1.5.2 remain immutable. This revision preserves
the count-invariant pixel-backed replacement mechanism but narrows eligibility
to geometry whose provenance is skew-tolerant or cadence-synthesized.

Parent-anchored horizontal-template geometry is intentionally not replaced:
those hypotheses already encode a regular five-line template, while a nearby
pixel-periodic candidate can be a different local five-line pattern. The gate
is generic and provenance-based; it does not use page identity, source family,
teacher coordinates, masks, holdout data, or qualification data.
"""
...
DETECTOR_VERSION = "stage11-v2d-restored-topology-successor.v1.5.3"
...
class RestoredTopologySuccessorV153Error(ValueError):
    pass


def _require(condition: bool, message: str) -> None:
    if not condition:
        raise RestoredTopologySuccessorV153Error(message)
...
def _eligible_for_pixel_replacement(item: StaffSystemDetection) -> bool:
    """Only replace skew/cadence geometry; never regular horizontal templates."""
    provenance = str(item.provenance)
    return (
        "skew-tolerant-horizontal-segment" in provenance
        or "parent-anchored-skew-segment" in provenance
        or "v1.5.1-interior-cadence-recovery" in provenance
    )


def _as_replacement(original: StaffSystemDetection, evidence: StaffSystemDetection) -> StaffSystemDetection:
    lines = tuple(
        StaffLineDetection(index, float(line.center_y), line.x_extent, line.segments)
    ...
    return StaffSystemDetection(
        ...
        lines,
        "restored-only:v1.5.3-pixel-backed-skew-cadence-replacement",
        DETECTOR_VERSION,
    )
...
def _pixel_backed_replacement(
    ...
    *,
    ...
    _require(len(reindexed) == len(ordered), "v1.5.3 replacement must preserve system count")
    return reindexed, {
        "eligibilityPolicy": "skew-or-cadence-only",
        "horizontalTemplateReplacementAllowed": False,
...
__all__ = [
    ...
    "RestoredTopologySuccessorV153Error",
    "StaffLineDetection",
