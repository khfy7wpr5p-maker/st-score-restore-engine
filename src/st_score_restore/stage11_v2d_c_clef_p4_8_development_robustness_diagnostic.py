"""P4.8 development-only C-clef robustness diagnostic.

Uses ONLY the frozen 29-page development corpus. The P4.7 holdout is never read.
Phase A generates raw candidates for original and synthetic scale variants before
teacher truth is opened. Phase B evaluates raw and frozen-P4.6-filtered candidates.
No detector thresholds are changed by this script.
"""
...
OUT = Path("/content/drive/MyDrive/ST_SCORE_RESTORE_STAGE11_EVAL/P4_C_CLEF_P4_8_DEV_DIAGNOSTIC")
RAW_PATH = OUT / "p4_8_development_scale_candidates.v1.json"
RESULT_PATH = OUT / "p4_8_development_scale_diagnostic.v1.json"
...
P46_EXPECTED_SOURCE_SHA256 = "33e05d64cdfdb32952887daae08cf623eb337a0ae544c154be763c923337e99e"
...
def load_frozen_module(name: str, url: str, expected_sha: str):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not create import spec for " + name)
...
def teacher_xyxy(box: Dict[str, Any], scale: float) -> List[float]:
    x = float(box["x"]) * scale
    y = float(box["y"]) * scale
    width = float(box.get("width", box.get("w"))) * scale
    height = float(box.get("height", box.get("h"))) * scale
...
def evaluate_variant(variant_pages: Dict[str, Any], teacher: Dict[str, Any], scale: float, filter_fn):
    raw_tp = raw_fp = raw_fn = 0
    filtered_tp = filtered_fp = filtered_fn = 0
    raw_subtype = Counter()
    filtered_subtype = Counter()
    ...
    for page_id, page in variant_pages.items():
        ...
        if page["diagnostics"]["detectedStaffCount"] == 0:
            zero_staff_pages += 1
        if not candidates:
            zero_candidate_pages += 1

        raw_matches = greedy_match(candidates, teacher_boxes, scale)
        raw_matched_t = {ti for _, ti, _ in raw_matches}
        raw_tp += len(raw_matches)
        raw_fp += len(candidates) - len(raw_matches)
        raw_fn += len(teacher_boxes) - len(raw_matches)
        for ti in raw_matched_t:
            raw_subtype[teacher_boxes[ti]["label"]] += 1

        accepted = [c for c in candidates if filter_fn(c)[0]]
        filtered_matches = greedy_match(accepted, teacher_boxes, scale)
        filtered_matched_t = {ti for _, ti, _ in filtered_matches}
        filtered_tp += len(filtered_matches)
        filtered_fp += len(accepted) - len(filtered_matches)
        filtered_fn += len(teacher_boxes) - len(filtered_matches)
        for ti in filtered_matched_t:
            filtered_subtype[teacher_boxes[ti]["label"]] += 1

    labels = ("C1", "C3", "C4")
    return {
        "zeroStaffPageCount": zero_staff_pages,
        "zeroCandidatePageCount": zero_candidate_pages,
        "raw": {
        ...
        "frozenP4_6Filtered": {
            "pooled": metric(filtered_tp, filtered_fp, filtered_fn),
            "subtypeRecall": {
                label: filtered_subtype[label] / subtype_total[label] if subtype_total[label] else 0.0
                for label in labels
            },
        },
    }
...
def main() -> None:
    ...
    print("=" * 72)
    print("P4.8 DEVELOPMENT ROBUSTNESS DIAGNOSTIC")
    print("P4.7 holdout: NOT READ")
    print("Teacher truth: NOT READ during Phase A candidate generation")
    print("=" * 72)

    raw_variants: Dict[str, Any] = {}
    with tempfile.TemporaryDirectory(prefix="p48_scale_") as temp_dir:
        temp_dir_path = Path(temp_dir)
        for variant_name, scale in VARIANTS.items():
            ...
            for page_index, page in enumerate(manifest["pages"], start=1):
                ...
                if scale == 1.0:
                    ...
                    work_path = temp_dir_path / f"{variant_name}-{page_id}.png"
                    if not cv2.imwrite(str(work_path), resized):
                        raise RuntimeError("Could not write synthetic image: " + page_id)

                candidates, diagnostics = p45.generate_page_candidates(work_path)
                variant_pages[page_id] = {
                    "pageId": page_id,
                    "scale": scale,
    ...
    raw_payload = {
        "schemaVersion": "stage11.v2d.c-clef-p4_8-development-scale-candidates.v1",
    ...
    print("PASS — Phase A frozen before teacher access")
    ...
    for variant_name, variant in raw_variants.items():
        diagnostics[variant_name] = evaluate_variant(
            variant["pages"], teacher, float(variant["scale"]), p46.filter_candidate
        )

    baseline_filtered_recall = diagnostics["scale_1_00"]["frozenP4_6Filtered"]["pooled"]["recall"]
    robustness = {}
    for variant_name, data in diagnostics.items():
        current_recall = data["frozenP4_6Filtered"]["pooled"]["recall"]
        robustness[variant_name] = {
            "filteredRecall": current_recall,
            "recallDropVsScale1": baseline_filtered_recall - current_recall,
        }

    result = {
        "schemaVersion": "stage11.v2d.c-clef-p4_8-development-scale-diagnostic.v1",
        "status": "COMPLETE",
        "developmentOnly": True,
        "p4_7HoldoutAccessed": False,
        "teacherTruthReadDuringCandidateGeneration": False,
    ...
    print()
    print("=" * 72)
    print("P4.8 DEVELOPMENT ROBUSTNESS DIAGNOSTIC COMPLETE")
    print("=" * 72)
    for variant_name, data in diagnostics.items():
        print(variant_name)
        print("  zeroStaffPages:", data["zeroStaffPageCount"])
        ...
        print("  recallDropVsScale1:", robustness[variant_name]["recallDropVsScale1"])
