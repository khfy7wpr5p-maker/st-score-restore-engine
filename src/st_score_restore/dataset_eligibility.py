...
def _storage_restriction_floor(permissions: dict[str, dict[str, Any]]) -> str:
    """Return the minimum class explicitly required by storage allowlists."""
    floor = "open_corpus"
...
def resolve_required_eligibility_class(
    *,
    ...
    temporal, retention, split, provider and non-storage operational gates remain
    separate. A caller may explicitly escalate to a stronger class, but never
    downgrade below the class returned here.
    """
    ...
    return _stronger(floor, _storage_restriction_floor(permissions))
...
def validate_declared_eligibility(
    *,
    ...
    if required == "blocked":
        ...
        if declared != "blocked":
            raise DatasetManifestError("metadata-only eligibility must remain blocked")
        return required
