#!/usr/bin/env python3
"""Run the non-production in-memory ST Score Restore `/api/v1` server."""
...
def main() -> int:
    ...
    service = RestorationJobService(InMemoryJobStore(), config)
    api = ApiV1(service, config)
    server, worker = create_server(args.host, args.port, api, service)
    worker.start()
    print(
        f"ST Score Restore non-production API listening on http://{args.host}:{args.port}",
        file=sys.stderr,
    )
    try:
