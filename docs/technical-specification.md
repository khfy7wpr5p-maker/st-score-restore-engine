...
ST Score Restore Engine is a safety-first visual restoration and validation service for music-score and guitar-TAB documents. It is not an OMR engine. Source bytes and historical evidence remain immutable; exact SHA-256 defines artifact identity; real corpus/derivative bytes remain outside ordinary Git; held-out evidence never tunes thresholds/resources; purpose, reference-label provenance and final acceptance rules fail closed.
...
`evidence/stage4/governance/purpose-grants.v1.json` is an immutable overlay; it does not rewrite Stage 1 catalog permissions. Exact Beethoven and Barley development artifacts are granted `safety_calibration`, constrained to `managed_standard`, `stage1_offline`, development split and no external export. Chopin remains held-out `held_out_evaluation` only with candidate derivation forbidden.

A purpose grant is **not** real calibration execution authorization. `realDataCalibrationExecutionAuthorized=false` remains until an accepted real reference-label bundle exists.

### 5.3 Reference-label contract

`src/st_score_restore/stage4_reference_labels.py` requires real labels to carry exact purpose permission plus human expert-review provenance. Model/prediction output cannot become reference truth. A real bundle requires separate acceptance.
...
Real calibration execution is **BLOCKED / NOT AUTHORIZED**. `calibrationAuthorized=false`, `realDataCalibrationExecuted=false`, `thresholdsCalibrated=false`, `resourceLimitsCalibrated=false`, `heldOutTuningUsed=false`, `modelTrainingAuthorized=false`, `publicationAuthorized=false`, `stage4ExitPass=false`, `stage5EntryEligible=false`.

No production Stage 2 quality threshold or Stage 3 page/resource limit changes are authorized by the purpose grant.
...
The next substantive step is an actual human-reviewed reference-label package for the authorized Beethoven/Barley development artifacts. Until separately accepted, no real calibration run or threshold/resource change is permitted. Stage 5 remains BLOCKED pending Stage 4 final exit PASS.
