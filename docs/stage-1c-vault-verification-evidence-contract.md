...
**Depends on:** Stage 1B custody/operations contract and the Stage 1C G4 pre-byte binding

## 1. Purpose

This contract defines the repository-visible evidence record used to determine whether a concrete offline Stage 1 custody environment has passed the minimum pre-byte operational verification required by Issue #47.
...
A vault-verification record can prove only the environment gate. It does not grant artifact-specific rights, privacy approval, dataset-review approval, purpose authorization, processing eligibility, or Stage 2 authorization.
...
- repository/CI wiring needed to make the new contract mandatory.
...
- declare the current vault compliant merely because the schema/validator exists;
...
- the Stage 1C G4 environment `stage1_offline`;
- the Stage 1C G4 storage class `custody_external`;
...
3. `offline_boundary` — the selected environment satisfies the approved offline boundary;
...
13. `git_and_sync_separation` — the vault remains outside ordinary Git and automatic cloud-sync folders.
...
C4 is complete only when all of the following are freshly demonstrated on the exact PR head:
...
The implementation must include at least these negative cases:
...
## 9. C4 completion boundary

Merging C4 would make the evidence format and fail-closed checks available. It would **not** make the current vault compliant.

The first real/private/copyrighted artifact remains blocked until a supported encrypted offline custody environment is actually exercised against the accepted Stage 1B controls, the resulting operational evidence is retained outside Git, a repository-visible C4 record validates as `pass`, and the specific artifact independently satisfies its Stage 1A rights, privacy, dataset-review, retention, and G4-purpose authorization gates.
